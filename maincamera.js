...
// @version      1.2.0
// @description  ADB SAFEGATE Safedock + Wing Camera Mini Player (A350-900 / 777-300ER only, Wing only)
// @author       chi_FUK77W
...
function isTargetAircraft(){
    try{
        ...
        // A350-900
        if(name.includes('A350') && (name.includes('900') || name.includes('A350-900') || name.includes('A350 900'))) return true;
        // 777-300ER
        if(name.includes('777') && (name.includes('300') || name.includes('300ER') || name.includes('777-300') || name.includes('777 300'))) return true;
        return false;
    }catch(e){
        return false;
    }
...
function findWingCamera(){
    ...
    if(!geofs?.camera?.modes) return -1;

    // 優先順位: 完全一致 "wing" → "wing" を含みかつ 2 を含まないもの
    let best = -1;
    for(let i=0; i<geofs.camera.modes.length; i++){
        const raw = (geofs.camera.modes[i].name || '');
        const name = raw.toLowerCase().trim();

        // Wing2 / Wing 2 を明示的に除外
        if(name.includes('wing2') || name.includes('wing 2') || name.endsWith('2') && name.includes('wing')) continue;
...
#vdgs-miniplayer{
    ...
    font-family:'Press Start 2P',monospace;cursor:pointer;overflow:hidden;
    user-select:none;
}
#vdgs-miniplayer .mp-header{
    background:#151515;padding:6px 10px;font-size:9px;color:#ffb000;
...
#vdgs-miniplayer .mp-body .title{font-size:11px;margin-bottom:8px;text-shadow:0 0 8px #ff8800}
#vdgs-miniplayer .mp-body .hint{font-size:8px;color:#aaa;font-family:sans-serif;line-height:1.4}
...
<div class="hint">Click to switch<br>A350 / 777 only</div>
</div>`;
document.body.appendChild(mp);

mp.onclick=()=>{
    if(wingCamMode>=0 && geofs?.camera){
        geofs.camera.set(wingCamMode);
    }
};
...
/* ===== Wing Mini Player (A350-900 / 777-300ER + Wing only) ===== */
