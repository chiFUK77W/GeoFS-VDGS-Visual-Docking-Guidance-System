...
// @version 1.3.3
// @description ADB SAFEGATE Safedock + Wing Camera Mini Player (A350 / 777-300ER, Wing only) - VT323 font
// @author chi_FUK77W
...
let previousCamMode = -1; // トグル用：前のカメラモードを記憶

function toRad(d){return d*Math.PI/180}
function toDeg(r){return r*180/Math.PI}
function fix360(a){return((a%360)+360)%360}
function relativeToDock(acLat,acLon,dockLat,dockLon,dockHdg){
const R=637 1000,dLat=toRad(acLat-dockLat),dLon=toRad(acLon-dockLon),meanLat=toRad((acLat+dockLat)/2);
...
function findWingCamera(){
 ...
 // "wing" を含むものを候補に（Left Wing / Right Wing / Wing View など）
...
function toggleWingCamera(){
 ...
 console.warn('[VDGS] Wing camera not found. Current modes:', geofs?.camera?.modes?.map(m => m.name));
 return;
 }

 if(geofs.camera.currentMode === idx){
 // すでにWingカメラなら前のカメラに戻す
 if(previousCamMode >= 0 && previousCamMode !== idx){
 geofs.camera.set(previousCamMode);
 }
 } else {
 // Wingカメラでない場合は現在のモードを記憶して切り替える
...
const f=document.createElement('link');f.rel='stylesheet';f.href='https://fonts.googleapis.com/css2?family=VT323&display=swap';document.head.appendChild(f);
const s=document.createElement('style');s.id='vdgs-style';
s.textContent=`#vdgs-panel{position:fixed;top:65px;right:18px;width:210px;background:#0a0a0a;border:3px solid #1c1c1c;border-radius:8px;box-shadow:0 12px 40px #000c;z-index:99999;overflow:hidden;font-family:'VT323',monospace;user-select:none}
#vdgs-header{background:#151515;padding:5px 9px;font-size:11px;color:#555;display:flex;justify-content:space-between;align-items:center;cursor:move;border-bottom:1px solid #222;font-family:'VT323',monospace}
#vdgs-header button{background:0;border:0;color:#777;font-size:16px;cursor:pointer;font-family:'VT323',monospace}
#vdgs-screen{background:#000;padding:14px 10px 16px;text-align:center}
#vdgs-slow{font-size:16px;color:#ffcc00;text-shadow:0 0 10px #ffaa00;min-height:18px;letter-spacing:2px;margin-bottom:2px;opacity:0;transition:opacity .15s}
#vdgs-slow.show{opacity:1}
#vdgs-type{font-size:26px;color:#ffb000;letter-spacing:1px;text-shadow:0 0 10px #ffb000cc;min-height:36px;line-height:1.3}
...
#vdgs-pos-arrow{color:#ffb000;font-size:18px;text-shadow:0 0 11px #ff8800;margin-top:4px;transition:transform .08s linear}
#vdgs-float{display:none;color:#ffb000;font-size:22px;text-shadow:0 0 12px #ff8800;animation:float-move 1.1s ease-in-out infinite}
.vdgs-side-arrow{position:absolute;top:28px;font-size:24px;color:#ff1a00;text-shadow:0 0 12px red;opacity:0;transition:opacity .1s}
.vdgs-side-arrow.active{opacity:1;animation:safedock-blink .36s infinite}
#vdgs-left{left:8px}#vdgs-right{right:8px}
# vdgs-stop-sign{display:none;width:118px;height:118px;background:#000;color:#ff1a00;font-size:22px;border:6px solid #ff1a00;box-sizing:border-box;clip-path:polygon(30% 0%,70% 0%,100% 30%,100% 70%,70% 100%,30% 100%,0% 70%,0% 30%);align-items:center;justify-content:center;margin:0 auto;box-shadow:0 0 20px #ff1a00b3;text-shadow:0 0 8px #ff1a00;line-height:1}
#vdgs-msg{font-size:18px;letter-spacing:2px;min-height:28px;margin-top:6px;text-shadow:0 0 10px currentColor}
#vdgs-msg.ok{color:#ffcc00}#vdgs-msg.info{color:#ffb000}
#vdgs-status{font-size:12px;color:#444;margin-top:6px;font-family:'VT323',monospace}
#vdgs-controls{display:flex;gap:3px;padding:5px;background:#0d0d0d;border-top:1px solid #1a1a1a}
#vdgs-controls button{flex:1;background:#1a1a1a;border:1px solid #2a2a2a;color:#999;padding:6px 0;font-size:12px;cursor:pointer;border-radius:3px;font-family:'VT323',monospace}
...
#vdgs-miniplayer .mp-header{
 background:#151515;padding:6px 10px;font-size:13px;color:#ffb000;
 display:flex;justify-content:space-between;align-items:center;
 border-bottom:1px solid #333;font-family:'VT323',monospace;
}
...
#vdgs-miniplayer .mp-body .title{font-size:15px;margin-bottom:8px;text-shadow:0 0 8px #ff8800}
#vdgs-miniplayer .mp-body .hint{font-size:12px;color:#aaa;font-family:'VT323',monospace;line-height:1.4}
...
window.addEventListener('keydown',e=>{
 ...
 e.preventDefault();
 const x=document.getElementById('vdgs-panel');
...
document.onmousemove=e=>{if(!drag)return;el.style.left=e.clientX-ox+'px';el.style.top=e.clientY-oy+'px';el.style.right='auto';el.style.bottom='auto';};
...
const msgEl=document.getElementById('vdgs-msg'),stemEl=document.getElement ById('vdgs-t-stem');
