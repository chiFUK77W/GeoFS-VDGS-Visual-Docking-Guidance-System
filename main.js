...
// @version      1.3.5
// @description  ADB SAFEGATE Safedock + Wing Camera Mini Player (A350 / 777-300ER) - VT323 / 文字大きめ / ドラッグ移動対応
// @author       chi_FUK77W
...
function injectCSS(){
    ...
    s.textContent = `
#vdgs-panel{
    position:fixed;top:65px;right:18px;width:250px;
...
#vdgs-header{
    background:#151515;padding:8px 12px;font-size:16px;color:#888;
    display:flex;justify-content:space-between;align-items:center;
    cursor:move;border-bottom:1px solid #222;
}
#vdgs-header button{
    background:0;border:0;color:#777;font-size:20px;cursor:pointer;
    font-family:'VT323',monospace;line-height:1;padding:0 4px;
}
#vdgs-screen{background:#000;padding:16px 12px 18px;text-align:center}
#vdgs-slow{
    font-size:20px;color:#ffcc00;text-shadow:0 0 10px #ffaa00;
    min-height:22px;letter-spacing:2px;margin-bottom:4px;
    opacity:0;transition:opacity .15s;
}
#vdgs-slow.show{opacity:1}
#vdgs-type{
    font-size:36px;color:#ffb000;letter-spacing:1px;
    text-shadow:0 0 12px #ffb000cc;min-height:42px;line-height:1.2;
}
#vdgs-guidance{
...
#vdgs-t-wrap{
    display:none;position:absolute;left:50%;top:10px;
    transform:translateX(-50%);flex-direction:column;align-items:center;
}
#vdgs-t-top{width:56px;height:12px;background:#ffb000;box-shadow:0 0 14px #ff8800}
#vdgs-t-stem{
    width:14px;background:#ffb000;box-shadow:0 0 14px #ff8800;
    transition:height .06s linear;margin-top:-1px;
}
#vdgs-pos-arrow{
    color:#ffb000;font-size:22px;text-shadow:0 0 11px #ff8800;
    margin-top:6px;transition:transform .08s linear;
}
#vdgs-float{
    display:none;color:#ffb000;font-size:28px;
    text-shadow:0 0 12px #ff8800;
    animation:float-move 1.1s ease-in-out infinite;
}
.vdgs-side-arrow{
    position:absolute;top:32px;font-size:30px;color:#ff1a00;
...
#vdgs-left{left:10px}
#vdgs-right{right:10px}
#vdgs-stop-sign{
    display:none;width:130px;height:130px;background:#000;color:#ff1a00;
    font-size:30px;border:7px solid #ff1a00;box-sizing:border-box;
    clip-path:polygon(30% 0%,70% 0%,100% 30%,100% 70%,70% 100%,30% 100%,0% 70%,0% 30%);
    align-items:center;justify-content:center;margin:0 auto;
    box-shadow:0 0 20px #ff1a00b3;text-shadow:0 0 8px #ff1a00;line-height:1;
}
#vdgs-msg{
    font-size:26px;letter-spacing:2px;min-height:32px;margin-top:8px;
...
#vdgs-controls{
    display:flex;gap:4px;padding:6px;background:#0d0d0d;
    border-top:1px solid #1a1a1a;
}
#vdgs-controls button{
    flex:1;background:#1a1a1a;border:1px solid #2a2a2a;color:#aaa;
    padding:8px 0;font-size:16px;cursor:pointer;border-radius:3px;
...
#vdgs-miniplayer{
    position:fixed;bottom:24px;left:24px;width:240px;height:140px;
...
#vdgs-miniplayer .mp-header{
    background:#151515;padding:8px 12px;font-size:15px;color:#ffb000;
    display:flex;justify-content:space-between;align-items:center;
...
#vdgs-miniplayer .mp-body .title{
    font-size:20px;margin-bottom:10px;text-shadow:0 0 8px #ff8800;
}
...
<div id="vdgs-t-stem" style="height:80px"></div>
...
function clearDock(){
    ...
    document.getElementById('vdgs-t-stem').style.height = '80px';
...
function updateDisplay(){
    ...
    if(along > TRACK_DIST){
        ...
        if(along <= STOP_DIST){
            ...
            const offset = Math.max(-38, Math.min(38, (cross / 5.5) * 38));
            posEl.style.transform = `translateX(${offset}px)`;

            let stemH = 80;
            if(along <= CLOSE_DIST)
                stemH = Math.max(1, ((along - STOP_DIST) / (CLOSE_DIST - STOP_DIST)) * 80);
